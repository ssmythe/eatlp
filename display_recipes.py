...
for recipe_key, recipe in recipes.dict_of_recipes.items():
    servings_per_recipe = (
        recipes.dict_of_recipes[recipe_key]
        .dict_of_recipe['servings_per_recipe']
    )
    serving_size = (
        recipes.dict_of_recipes[recipe_key].dict_of_recipe['serving_size']
    )
    print(
        f"Recipe: {recipe_key} (makes {servings_per_recipe} servings of "
        f"{serving_size})"
    )
    print('-' * 114)
    ...
    print(
        ...
        (
            recipe_key, kcal, carb, fat, protein, sodium, fiber,
            price, priceperkcal, cent_symbol
        )
    )

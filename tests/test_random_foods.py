...
def test_foods_to_randomfoods(foods, randomfoods):
    ...
    assert sorted(randomfoods.dict_of_random_foods.keys()) == \
        sorted(['chobani yogurt with honey', 'cheerios and almondmilk',
                'ribeye steak', 'chobani yogurt', 'potato with vbutter',
                'split pea soup', 'toast with earth balance',
                'toast with chao', 'salad with vranch dressing',
                'honeycrisp apple'])
...
def test_foods_to_randomfoods_two(foods, randomfoods):
    ...
    assert sorted(randomfoods.dict_of_random_foods.keys()) == \
        sorted(['black beans and onions', 'cheerios and almondmilk',
                'tofurky chao mayo sandwich', 'special k and almond milk',
                'toast with chao', 'toast with just egg', 'toast with hummus',
                'salad with vranch dressing', 'chobani yogurt with honey',
                'split pea soup', 'chobani yogurt', 'almondmilk',
                'salad with anlvr dressing', 'green beans', 'yellow peach'])
